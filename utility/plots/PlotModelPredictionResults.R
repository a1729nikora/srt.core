...
plot_model_prediction_results <- function(modelchoicelist,ModelsThatRan,plotType,data,ResultsOfModels,C1,C2,C3,lifecycle,input) {
  ConfIntSuffixes <- c("Low", "MLE", "High")
  
  DisplayOnQueryPlot <- list("Low"=input$LowConfOnQueryPlot, "MLE"=input$MLEOnQueryPlot, "High"=input$HighConfOnQueryPlot)
  ...
  for (model in modelchoicelist){
    for(SuffixTag in ConfIntSuffixes) {
      if(any(ModelsThatRan[[SuffixTag]] == model)) {
        # Get model parameters from model results
        
        last_row <- length(ResultsOfModels[,1]) - PredAheadSteps
        ...
        for (paramNum in 1:length(get(model_params_label))) {
          ...
          model_params[1,paramNum] <- ResultsOfModels[[model_parm_num]][last_row]
        }
